...
使用**差错检测**技术可以检查帧在传输过程中是否发送了误码，数据链路层向上层提供了两种服务：

+ 不可靠传输：仅仅丢弃有误码的帧，其他什么也不做
+ 可靠传输：想办法实现发送方发送什么，接收方就收到什么

一般情况下，**有线链路**的误码率比较低，一般不会要求提供可靠传输服务，**无线链路**误码率较高，要求必须向上层提供可靠传输服务

传输差错不仅有比特差错，还有分组失序，分组丢失，分组重复等

可靠传输服务并不局限于数据链路层，其他各层都可选择实现可靠传输



**停止等待协议**

发送一个分组后，接收到确认分组的信息才发送下一个分组，如果收到拒绝分组的信息就重新发送分组



**回退N帧协议**

以一个窗口为单位发送分组，进行累计确认，如果期间有一个分组发生了误码可能会造成其他分组的牵连，每收到一个错误的分组就发送最新的确认分组



**选择重传协议**

按顺序到达的分组会使窗口向前滑动，如果没有按顺序到达，窗口就不会到达，只有当分组填满了窗口，窗口才会向前滑动
